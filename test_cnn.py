...
from other_models.cnn_lstm import CNNLSTM
...
def main(mode = None, load_model=False):
    ...
    print('Create Model')
    model = CNNLSTM(num_classes=len(class_labels))
    # Lightning Callbacks
    wandb_logger = WandbLogger(project="cnn-lstm-wandb")
    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss",
        mode="max",
        dirpath="./wandb_checkpoints",
        filename=f"cnn_lstm_b{args.batch_size}_lr{args.lr}",
    )
